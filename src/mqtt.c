...
//Remaining Length must be at most 4 bytes
static const int MAX_LEN_BYTES = 4;

//Encode Remaining Length of the packet
...
static size_t unpack_mqtt_connect(const unsigned char *buf, union mqtt_header *hdr, union mqtt_packet *pkt)
{
    ...
    //Second byte contains the length of remaining bytes
    ...
    //read thewill topic and message 
...
static size_t unpack_mqtt_subscribe(const unsigned char *buf, union mqtt_header *hdr, union mqtt_packet *pkt){
    ...
    while(remaining_bytes > 0){
        //read the length bytes of the first topic filter
        remaining_bytes -= sizeof(uint16_t);

        //we have to make room for aditional tuples
